...
fn adjust_path(app_handle: &AppHandle, appid: &str, path: &str, grid_type: &str) -> String {
    let format_start_index = path
        .rfind(".")
        .expect("Path should have had a file extension.");
...
fn filter_paths(
    ...
    current_paths: &GridImageCache,
    original_paths: &GridImageCache,
    ...
    for (appid, grids_map) in current_paths.into_iter() {
        for (grid_type, source_path) in grids_map.into_iter() {
            ...
            if original_paths.get(appid.as_str()).is_some()
                && original_paths
                    .get(appid.as_str())
                    .unwrap()
                    .get(grid_type.as_str())
                    .is_some()
            {
                grid_path = original_paths
                    .get(appid.as_str())
                    .unwrap()
                    .get(grid_type.as_str())
                    .unwrap();
            ...
            if source_path_owned != grid_path_owned {
                ...
                if source_path != "REMOVE" {
                    let adjusted_path = adjust_path(
                        app_handle,
                        appid.as_str(),
                        source_path_owned.as_str(),
                        grid_type.as_str(),
                    )
                    .replace("\\", "/");
                    if grid_type == "Icon" && !is_appid_shortcut(&appid, shortcut_icons) {
                        ...
                        target_path = String::from(grids_dir.join(adjusted_path).to_str().unwrap())
                            .replace("\\", "/");
                    }
                } else {
...
fn check_for_shortcut_changes(
    ...
    for (shortcut_id, icon) in shortcut_icons.to_owned().into_iter() {
        let icon: &str = icon
            .as_str()
            .expect("Should have been able to convert icon to &str.");
        let original_icon: &str = original_shortcut_icons
            .get(&shortcut_id)
            .expect("Original shortcut should have had an icon.")
            .as_str()
            .expect("Should have been able to convert original icon to &str.");
...
pub async fn save_changes(
    ...
    let paths_to_set: Vec<ChangedPath> = filter_paths(
        &app_handle,
        steam_path.to_owned(),
    ...
    let paths_id_map: HashMap<String, ChangedPath> = paths_to_set
        .clone()
        .iter()
        .map(|entry| {
            (
                format!("{}_{}", entry.appId.to_owned(), entry.gridType.to_owned()).to_string(),
                entry.to_owned(),
            )
        })
        .collect();
    ...
    for (appid, steam_logo_str_val) in changed_logo_positions.into_iter() {
        let steam_logo_str: &str = steam_logo_str_val
            .as_str()
            .expect("Should have been able to convert steamLogo pos into str.");
    ...
    if should_change_shortcuts {
        ...
        let mut shortcuts_data: Value = serde_json::from_str(shortcuts_str.as_str())
            .expect("Should have been able to parse json string.");

        let shortcuts_obj_map: &mut Value = shortcuts_data
            .get_mut("shortcuts")
            .expect("key: shortcuts should have existed.");
        let shortcuts_map: &mut Map<String, Value> = shortcuts_obj_map
            .as_object_mut()
            .expect("Should have been able to convert shortcuts to map");

        for (_, shortcut) in shortcuts_map.into_iter() {
            let shortcut_map: &mut Map<String, Value> = shortcut
                .as_object_mut()
                .expect("should have been able to convert shortcut to map.");
            let shortcut_appid_val: &Value = shortcut_map
                .get("appid")
                .expect("shortcut should have had an appid");
            let shortcut_appid_num: i64 = shortcut_appid_val
                .as_i64()
                .expect("should have been able to convert shortcut appid to str.");
            ...
            if paths_id_map.contains_key(&path_key) {
                let changed_path: &ChangedPath = paths_id_map
                    .get(&path_key)
                    .expect("entry should have existed.");
                shortcut_map.insert(
                    String::from("icon"),
                    Value::String(changed_path.targetPath.to_owned()),
                );
            }
        }
...
pub async fn write_shortcuts(
    ...
    let shortcuts_data: Value = serde_json::from_str(shortcuts_str.as_str())
        .expect("Should have been able to parse json string.");
